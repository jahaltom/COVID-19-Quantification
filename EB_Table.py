...
for i in files:
    ...
    combos.append([i.replace('_EB.tsv' , ''),(len([col for col in df if col.startswith('SRR')]) or len([col for col in df if col.startswith('ERR')])),len(df[(df['Gene_type'] == 'EB_novel') & (df['is54K_EB'] == False)]),df['is54K_EB'].value_counts()[1] ,len(df[(df['final_strata'] == "Homo sapiens") & (df['is54K_EB'] == False) & (df['Gene_type'] == 'EB_novel')]),len(df[(df['final_strata'] == "Homo sapiens") & (df['is54K_EB'] == True)])])
